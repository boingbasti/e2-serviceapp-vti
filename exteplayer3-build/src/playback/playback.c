...
static int32_t PlaybackStop(Context_t  *context) 
{
    ...
    RAW_DEBUG_LOG( "[exteplayer3] PlaybackStop: enter\n");
    ...
    if (context && context->playback && context->playback->isPlaying) 
    {
        ...
        RAW_DEBUG_LOG( "[exteplayer3] PlaybackStop: calling OUTPUT_STOP\n");
        context->output->Command(context, OUTPUT_STOP, NULL);
        RAW_DEBUG_LOG( "[exteplayer3] PlaybackStop: calling CONTAINER_STOP\n");
        context->container->selectedContainer->Command(context, CONTAINER_STOP, NULL);
        RAW_DEBUG_LOG( "[exteplayer3] PlaybackStop: CONTAINER_STOP done\n");
    }
    else
    {
        RAW_DEBUG_LOG( "[exteplayer3] PlaybackStop: stop not possible (already stopped or not playing)\n");
        ret = cERR_PLAYBACK_ERROR;
    }

    RAW_DEBUG_LOG( "[exteplayer3] PlaybackStop: done\n");
