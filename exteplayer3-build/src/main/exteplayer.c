...
{   
    ...
    while ( (c = getopt(argc, argv, "G:W:H:A:V:U:we3dlsrimvCa:n:x:u:c:h:o:p:P:t:9:0:1:4:5:6:7:f:b:F:S:O:T:L:Q:D:g:")) != -1)
    {
        ...
        case 'P':
            ...
            break;
        case 'Q':
            printf("HLS start quality mode: %d (0=auto,1=lowest,2=highest)\n", atoi(optarg));
            hls_quality_mode_set(atoi(optarg));
            break;
        case 'D':
            printf("HLS audio default-only filter: %d\n", atoi(optarg));
            hls_audio_default_only_set(atoi(optarg));
            break;
        case 'g':
...
//    commandRetVal = g_player->playback->Command(g_player, PLAYBACK_OPEN, &playbackFiles);
    ...
    
    {
        ...
        while(g_player->playback->isPlaying && 0 == PlaybackDieNow(0))
        {
            ...
            case 'q':
            {
                RAW_DEBUG_LOG("[exteplayer3] Main loop: received 'q' command, calling PLAYBACK_STOP\n");
                commandRetVal = g_player->playback->Command(g_player, PLAYBACK_STOP, NULL);
                E2iSendMsg("{\"PLAYBACK_STOP\":{\"sts\":%d}}\n", commandRetVal);
                RAW_DEBUG_LOG("[exteplayer3] Main loop: PLAYBACK_STOP command returned %d\n", commandRetVal);
                break;
            }
            case 'c':
            {
        ...
        RAW_DEBUG_LOG("[exteplayer3] Main loop exited, calling OUTPUT_CLOSE\n");
        g_player->output->Command(g_player, OUTPUT_CLOSE, NULL);
        RAW_DEBUG_LOG("[exteplayer3] OUTPUT_CLOSE done\n");
    }
    ...
    RAW_DEBUG_LOG("[exteplayer3] exteplayer3 process calling exit(0)\n");
    exit(0);
}
