...
struct CustomIOCtx_t;
extern struct CustomIOCtx_t* custom_io_tab[];
...
static int isEOFReached[2] = {0, 0};

static int is_network_stream(int idx)
{
    if (idx < 0 || idx >= 2)
    {
        return 0;
    }
    if (avContextTab[idx] && avContextTab[idx]->url)
    {
        if (strncmp(avContextTab[idx]->url, "http://", 7) == 0 || 
            strncmp(avContextTab[idx]->url, "https://", 8) == 0)
        {
            return 1;
        }
    }
...
static int8_t is_finish_timeout(int idx)
{
    int max_iters = TIMEOUT_MAX_ITERS;
    if (is_network_stream(idx))
    {
        max_iters = 100; // 10 seconds for network streams (both Live and VOD)
    }

    if (finishTimeout > max_iters)
    {
        return 1;
    }
    return 0;
...
static void update_finish_timeout(int idx)
{
    if(0 == pauseTimeout)
    {
        ...
        /* For local files, if the buffer runs dry, it's always EOF.
         * For network streams (VOD), only close immediately if we actually reached EOF (isEOFReached is true).
         * For network live streams, isEOFReached is false, so it never closes immediately.
         */
        int should_close_immediately = 0;
        if (!is_network_stream(idx))
        {
            should_close_immediately = 1;
        }
        else if (isEOFReached[idx])
        {
            should_close_immediately = 1;
        }

        if( should_close_immediately && !progressive_playback && 0 == ret && currPts >= maxInjectedPts && 
...
static int32_t ffmpeg_read_wrapper_base(void *opaque, uint8_t *buf, int32_t buf_size, uint8_t type)
{
    ...
    if(0 == PlaybackDieNow(0))
    {
        ...
        int idx = (opaque == custom_io_tab[1]) ? 1 : 0;
        while(len < buf_size && g_context && 0 == PlaybackDieNow(0))
        {
            ...
            else if (is_finish_timeout(idx))
            {
                len = 0;
                break;
            }
            
            update_finish_timeout(idx);
