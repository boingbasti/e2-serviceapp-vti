...
int LinuxDvbClose(Context_t  *context, char *type)
{
    ...
    RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbClose: enter for type='%s'\n", type);
    ...
    if (video && videofd != -1)
    {
        RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbClose: closing videofd=%d\n", videofd);
        close(videofd);
        videofd = -1;
    }
    if (audio && audiofd != -1) 
    {
        RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbClose: closing audiofd=%d\n", audiofd);
        close(audiofd);
        audiofd = -1;
    }

    releaseLinuxDVBMutex();
    RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbClose: done for type='%s'\n", type);
    return cERR_LINUXDVB_NO_ERROR;
}
...
int LinuxDvbStop(Context_t  *context __attribute__((unused)), char * type) 
{
    ...
    RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: enter for type='%s'\n", type);

    getLinuxDVBMutex();

    if (video && videofd != -1) 
    {
        RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: stopping video (videofd=%d)\n", videofd);
        if (ioctl(videofd, VIDEO_CLEAR_BUFFER) == -1)
        {
            RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: VIDEO_CLEAR_BUFFER failed: errno=%d (%s)\n", errno, strerror(errno));
        }
        
        if (ioctl(videofd, VIDEO_STOP) == -1)
        {
            RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: VIDEO_STOP failed: errno=%d (%s)\n", errno, strerror(errno));
            ret = cERR_LINUXDVB_ERROR;
        }

        ioctl(videofd, VIDEO_SLOWMOTION, 0);
        ioctl(videofd, VIDEO_FAST_FORWARD, 0);
        int res_source = ioctl(videofd, VIDEO_SELECT_SOURCE, VIDEO_SOURCE_DEMUX);
        RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: reset video source to DEMUX, ioctl returned %d (errno=%d)\n", res_source, errno);
    }
    if (audio && audiofd != -1) {
        RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: stopping audio (audiofd=%d)\n", audiofd);
        if (ioctl(audiofd, AUDIO_CLEAR_BUFFER) == -1)
        {
            RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: AUDIO_CLEAR_BUFFER failed: errno=%d (%s)\n", errno, strerror(errno));
        }

        if (ioctl(audiofd, AUDIO_STOP) == -1)
        {
            RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: AUDIO_STOP failed: errno=%d (%s)\n", errno, strerror(errno));
            ret = cERR_LINUXDVB_ERROR;
        }
        int res_source = ioctl(audiofd, AUDIO_SELECT_SOURCE, AUDIO_SOURCE_DEMUX);
        RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: reset audio source to DEMUX, ioctl returned %d (errno=%d)\n", res_source, errno);
    }

    releaseLinuxDVBMutex();

    RAW_DEBUG_LOG( "[exteplayer3] LinuxDvbStop: done for type='%s'\n", type);
    return ret;
}
