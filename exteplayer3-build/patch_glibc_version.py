#!/usr/bin/env python3
"""Patcht neuere GLIBC-Versionen auf glibc 2.21-kompatible Versionen (ARM 32-bit LE)."""
import sys
import struct

# (alt, neu) — müssen exakt gleiche Bytezahl inkl. Null-Terminator haben
VERSION_MAP = [
    (b'GLIBC_2.38\x00', b'GLIBC_2.4\x00\x00'),
    (b'GLIBC_2.35\x00', b'GLIBC_2.4\x00\x00'),
    (b'GLIBC_2.34\x00', b'GLIBC_2.4\x00\x00'),
    (b'GLIBC_2.33\x00', b'GLIBC_2.4\x00\x00'),
    (b'GLIBC_2.29\x00', b'GLIBC_2.4\x00\x00'),
    (b'GLIBC_2.27\x00', b'GLIBC_2.4\x00\x00'),
]
...
def patch(path):
    ...
    if not verneed_off or not strtab_off:
        print(f"FEHLER: VERNEED oder .dynstr nicht gefunden in {path}")
        sys.exit(1)
    ...
    for OLD, NEW in VERSION_MAP:
        ...
        if patched_strings or patched_hashes:
            print(f"  {old_ver} → GLIBC_2.4: {patched_strings} String(s), {patched_hashes} Hash(es)")
        total_strings += patched_strings
        total_hashes  += patched_hashes

    if total_strings == 0 and total_hashes == 0:
        print(f"{path}: keine neuen GLIBC-Versionen gefunden — nichts geändert")
        return

    with open(path, 'wb') as f:
        f.write(data)
    print(f"{path}: {total_strings} String(s), {total_hashes} Hash(es) gepatcht → GLIBC_2.4")
