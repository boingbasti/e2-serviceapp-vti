	...
	public iSubtitleOutput, public iSubserviceList, public iCueSheet, public iRdsDecoder
{
	...
	SigC::Slot2<void,iPlayableService*,int> m_event_slot;
	...
	SubtitleTrack m_selected_subtitle_track;
	bool m_has_selected_subtitle_track;
	...
	Event *m_event_now_raw;
	Event *m_event_next_raw;
	void updateEpgCacheNowNext();
#endif
	void gotExtPlayerMessage(int message);

	// Static wrappers für bind-basierte Slots (SigC::Object ABI-Umgehung)
	// bind() bindet das LETZTE Argument → self* muss LETZTER Parameter sein
	static void s_pushSubtitles(eServiceApp *self) { self->pushSubtitles(); }
	static void s_signalEventUpdatedInfo(eServiceApp *self) { self->signalEventUpdatedInfo(); }
	static void s_gotExtPlayerMessage(int msg, eServiceApp *self) { self->gotExtPlayerMessage(msg); }
#ifdef HAVE_EPG
	static void s_updateEpgCacheNowNext(eServiceApp *self) { self->updateEpgCacheNowNext(); }
#endif

	eTimer *my_subtitle_sync_timer;
	eTimer *my_event_updated_info_timer;
#ifdef HAVE_EPG
...
public:
	...
	RESULT record(const eServiceReference &ref, ePtr<iRecordableService> &ptr);
