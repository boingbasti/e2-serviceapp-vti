...
std::vector<std::string> ExtEplayer3::buildCommand()
{
	...
	if (!headersStr.empty())
	{
		SALOG("exteplayer3 headers: %s", headersStr.c_str());
		args.push_back("-h");
	...
	if (mPlayerOptions.downmix)
		args.push_back("-s");
	if (mPlayerOptions.hlsQualityMode != 0)
	{
		args.push_back("-Q");
		std::stringstream sstm;
		sstm << mPlayerOptions.hlsQualityMode;
		args.push_back(sstm.str());
	}
	if (mPlayerOptions.hlsAudioDefaultOnly)
	{
		args.push_back("-D");
		args.push_back("1");
	}
	if (mPlayerOptions.debugLoggingEnabled)
	{
		args.push_back("-g");
		args.push_back("1");
	}
